...
async def run_simulation(body: dict):
    attack_type = body.get("attack")
    if not attack_type:
        raise HTTPException(status_code=400, detail="Missing attack type")
    
    from backend.utils.testing.simulate import (
        simulate_port_scan, simulate_ssh_bruteforce, simulate_syn_flood,
        simulate_http_flood, simulate_sql_injection, simulate_steg_image,
        simulate_steg_video, simulate_apt_full, simulate_all
    )
    ...
    sim_fn = sim_map.get(attack_type)
    if not sim_fn:
        raise HTTPException(status_code=400, detail=f"Unknown attack type: {attack_type}")
    
    sim_fn()
    return {"status": "success", "message": f"Simulation triggered: {attack_type}"}
