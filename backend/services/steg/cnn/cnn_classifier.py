...
def predict_image(img_array: np.ndarray) -> float:
    """Legacy helper for detector.py. Returns CNN prediction confidence."""
    if not _try_load_model():
        return 0.5
    try:
        return _run_efficientnet(img_array)
    except Exception as e:
        logger.warning(f"[CNN] predict_image error: {e}")
        return 0.5


_RF_MODEL_PATH = Path("models/steg_model.pkl")
_rf_model = None

def predict_rf(img_np: np.ndarray) -> float:
    """Run Random Forest classifier on image array, return P(steganographic)."""
    global _rf_model
    if _rf_model is None:
        if not _RF_MODEL_PATH.exists():
            logger.warning(f"[RF] Model not found at {_RF_MODEL_PATH}. Returning fallback 0.5.")
            return 0.5
        try:
            import joblib
            _rf_model = joblib.load(_RF_MODEL_PATH)
            logger.info(f"[RF] Random Forest loaded from {_RF_MODEL_PATH}")
        except Exception as e:
            logger.warning(f"[RF] Failed to load RF model: {e}")
            return 0.5
    ...
    try:
        ...
        # LSB ratio
        ones = np.sum(pixels & 1)
        lsb_ratio = float(ones / total) if total > 0 else 0.0

        # Histogram variance
        hist_variance = float(np.var(histogram))

        # Mean and Standard Deviation
        mean_val = float(np.mean(pixels))
        std_dev = float(np.std(pixels))

        # Edge density (vectorized)
        if pixels_2d.shape[0] > 1 and pixels_2d.shape[1] > 1:
            diffs = np.abs(pixels_2d[:-1, :-1].astype(int) - pixels_2d[:-1, 1:].astype(int))
            edges = np.sum(diffs > 20)
            edge_density_val = float(edges / (pixels_2d.shape[0] * pixels_2d.shape[1]))
        else:
        ...
        logger.warning(f"[RF] predict_rf error: {e}")
        return 0.5
